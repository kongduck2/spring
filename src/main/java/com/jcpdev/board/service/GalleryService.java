...
public class GalleryService {
	...
	public int save(Gallery vo) throws IllegalStateException, IOException {
	      ...
	      if (files != null && files.size() > 0) { //첨부된 파일이 있을 때
	            for (MultipartFile f : files) {	//첨부된 파일리스트에 하나씩
	               ...
	               String newpath="";
	             //String fileName = "board_" + f.getOriginalFilename();   //board_원래 파일명.확장자 ,파일명 변경하여 db에 저장
	...
	public String randomString(String oldfile) { //랜덤 문자열 (숫자10개, 알파벳대소문자 사용) 원하는 length로 생성
	      ...
	      //확장자 추출 oldfile이 xxxxx.png xxxxx변경함
	      ...
	      //또는 UUID.randomUUID().toString(); <- 생성 문자열이 길다
	      return rString+ext;
	   }
